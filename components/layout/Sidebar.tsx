...
function NavLink({
  ...
  return (
    <Link
      href={href}
      className={cn(
        "flex items-center gap-2.5 rounded-md py-2.5 pl-3 pr-3 text-sm transition-colors duration-150 ease-out",
        "border-l-[3px]",
        active
    ...
    >
      <Icon
        className={cn(
          ...
          active ? "text-primary" : primary ? "text-primary/90" : "text-muted-foreground",
        )}
      />
      {label}
    </Link>
  ...
  return (
    <aside className="flex h-full w-[240px] shrink-0 flex-col border-r border-border bg-card">
      ...
      <div className="border-t border-border px-3 py-3">
        <div
          className="flex items-center gap-2 rounded-md border border-border bg-muted/40 px-2.5 py-2 text-[10px] font-medium leading-none text-muted-foreground"
          role="status"
        >
